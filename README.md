...
**Codex.** Run `codex mcp login kabo --scopes openid,offline_access,account:read,registry,telemetry,data` and complete the OAuth authorization in the browser. The `--scopes` list is not optional: without it the host registers with a default scope set the platform rejects with `invalid_scope`, before you ever reach the consent page. The host holds and renews the token; no Kabo credential is stored on your machine. Only the Claude variant signs in from the terminal.
