...
To log out of **this machine**: run `$kabo-logout`. It runs `codex mcp logout kabo` — the host drops its copy of the token, which is the same-second cutoff — and clears the local cache. It touches nothing over the network, and the account's access on other devices is left exactly as it was.

To revoke authorization **account-wide** (a lost machine, "sign me out everywhere"): run `$kabo-revoke`. It calls the platform's `auth_revoke_all` over the already-connected MCP link, destroying every device's grant at once, and then logs this machine out as above. Elsewhere it is not instant: an access token another host already holds is a self-contained JWT and the platform runs no denylist, so that machine keeps working for up to **2 more hours**, after which nothing can be renewed.
...
- `$kabo-logout`: logs out **this machine only** — `codex mcp logout kabo`, then clears the Codex skill cache, the run work directories, and trust material. Other devices are untouched.
- `$kabo-revoke`: the account-wide one — revokes every device's authorization through the platform's `auth_revoke_all` tool, then does everything `$kabo-logout` does.
...
**Envelope staging on Codex.** Codex 0.154 parses `PostToolUse`. `hooks/hooks.json` keeps SessionStart and adds that one `command` hook for `data_connector_(run|batch_run|job|artifact)$`. The host still parses handlers as `command` | `prompt` | `agent`; never add `PostToolUseFailure` (not in the Codex event enum) or `mcp_tool` — an unknown event or type disables the whole file and silently kills SessionStart (incident 2026-08-18). A `kabo:` line names the staging directory; pass it as `--staging`. Hand-write `work/<run-id>/snapshot/envelope-NN.json` and omit `--staging` only when nothing was staged.
...
**Tool-level telemetry is recorded by the server itself**: inside the MCP tool handler the platform already holds the tool name, your user id (from the verified JWT), the duration, and success or failure, and writes them straight to its database. The client has no telemetry PreToolUse/PostToolUse hooks and no local telemetry buffer — that "local JSONL + REST batch upload" channel was removed entirely in 0.9.0 (the platform-side `POST /api/telemetry` is retired as well). The PostToolUse hook that remains writes connector envelopes to local disk only and returns identifiers, never envelope content and never a network call.
