...
1. Resolve the plugin root two levels up from this `SKILL.md` path. An attached cross-client convention may name `${CLAUDE_PLUGIN_ROOT}`, the Claude-only `~/.kabo/plugin-root` directory, or `$KABO_DATA_ROOT/plugin-root`; on Codex these are path placeholders, not literal locations (this client never reads `KABO_DATA_ROOT`). Normalize all of them to this resolved plugin root (which is also recorded as one line in `<data root>/plugin-root`). Inside a `kabo-run-pipeline` step all are the `{plugin}` placeholder.
...
- **Artifact bodies.** A finished job lists its products as `object_ref` + `sha256` + `bytes` — identifiers, not content. Reading the bytes is a separate call: `data_connector_artifact` with that `job_id` and the `object_ref` values you want. Keyframes come back as images you can look at; a transcript comes back as text. Ask for the frames the current step actually needs — each one spends context, and `frames[]` carries a `t_seconds` for every frame so the moments can be chosen before they are fetched. A frame that is listed but never read is not evidence you have seen.
