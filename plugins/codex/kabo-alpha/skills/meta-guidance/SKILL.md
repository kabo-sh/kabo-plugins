...
- **Data root**: `$KABO_CODEX_DATA`, falling back to `~/.kabo/codex`. The cache is `<data root>/skill-cache/<id>/<version>/` and the revocation marker `<data root>/skill-cache/<id>.disabled`. The snapshot's `$KABO_DATA_ROOT` is the Claude variant's data-root variable and this client never reads it: wherever the snapshot says `$KABO_DATA_ROOT/skill-cache` or `$KABO_DATA_ROOT/plugin-root` ("falling back to `~/.kabo`"), read `<data root>/skill-cache` and `<data root>/plugin-root` as defined here — never a bare `/skill-cache` or `/plugin-root` from an unset variable, and never `~/.kabo` itself.
