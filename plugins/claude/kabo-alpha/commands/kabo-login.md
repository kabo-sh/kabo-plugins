...
**Say the activation step before starting, not after**: signing in is once per machine, but the session it happens in does not pick the credential up. The host asks the plugin's credential helper for it exactly once, when it connects the `kabo` server at session start; a session that connected before the sign-in was told there is no credential, and the host does not ask again for the life of that session (after a 401 it runs its own OAuth discovery, never the helper). So tell the user up front: confirm the code, then start a new session — and if the host shows an "Authenticate" / "needs authentication" prompt for `kabo`, ignore it (section 3 says why).

Requires **Claude Code 2.1.195 or newer** — see the troubleshooting section, this is the first thing to check when sign-in succeeded but the tools stay invisible in a **new** session.
...
## 2. Say the activation step, then verify with a real call

Sign-in does **not** take effect in the session it happened in: the host asked the credential helper once, when it connected the `kabo` server, and does not run it again once it has marked the server as needing authentication (a 401 mid-session sends the host into its own OAuth discovery, not back to the helper). So the moment `--wait` exits 0, say two things: the sign-in is done and was the once-per-machine step, and the tools appear in a **new session** (section 3 has the host-specific wording). Then confirm instead of assuming — the session may already hold a working connection from an earlier sign-in (a re-login to switch accounts, or a session that started with a credential on disk): call `mcp__plugin_kabo-alpha_kabo__registry_skill_search` with query `youtube`.
...
- Tool invisible, or the call answers 401 → the expected outcome of a first sign-in, not a failure. Repeat the activation line from section 3 and stop: do not retry the call in a loop, and do not send the user to the host's Authenticate prompt. The onboarding above runs in the new session — tell them to run `/kabo-start` there (or simply state a request). Still unavailable in the new session → troubleshooting.

## 3. Activate: a new session (the CLI can reconnect instead)

Say it in this order, host-agnostic first:

1. **"Start a new session — the tools appear there."** That works on every host — terminal, IDE extension, and the desktop app (Cmd/Ctrl+N there) — and a new session re-runs the credential helper with the fresh credential. This is the whole advice for a desktop or IDE user; never route them to the host's own "Authenticate"/OAuth prompt for `kabo` (the plugin is the auth path, and the host's OAuth leaves a token the plugin cannot manage).
...
- If the user asks how to **log out**: run `/kabo-logout`. It deletes this machine's credential and local data, and leaves every other device signed in. If they ask how to **revoke** authorization account-wide — everywhere at once, e.g. after losing a machine — that is `/kabo-revoke`: it revokes every device through the platform first, then logs this machine out too.
