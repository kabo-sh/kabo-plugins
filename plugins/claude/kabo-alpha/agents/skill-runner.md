...
- **Readiness**: the readiness note settles it; only when none was attached, call `data_connector_catalog` once with `skill_id` or SKILL.md’s explicit `connector_ids`; an empty skill filter requires the explicit IDs. A connector not `ready` or an operation not `implemented` is a **platform-side gap** — report it with the response's `setup_hint`, stop that evidence path, never send the user to configure a key. The `required.tools` check still applies.
