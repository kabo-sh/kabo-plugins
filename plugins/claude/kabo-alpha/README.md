...
| `/kabo-logout` | Logs out **this machine only**: deletes this machine's credential along with the cache, run work directories and trust material. It touches nothing over the network, so it never fails on a bad connection and is complete the second it returns — and the account's authorization on other machines, in other MCP clients and in the desktop app is left exactly as it was |
| `/kabo-revoke` | The account-wide one, for a lost or stolen machine: it calls the platform's `auth_revoke_all` over the authorized MCP connection — destroying **every device's** grant, not just this machine's — and then runs the same local logout here. The order cannot be swapped: deleting the credential first leaves nothing to call the platform with |

Which one is which: `/kabo-logout` is the everyday command and stops at this machine's edge; `/kabo-revoke` is the one that reaches the account, and every other device then needs a fresh `/kabo-login`. Splitting them is deliberate — it matches the desktop app, whose sign-out is likewise local-only — so that "log out of this laptop" can never quietly sign the user out of their phone.

How fast each surface stops **after `/kabo-revoke`**: **this machine, the same second** (the credential is gone, so no header is produced and the request 401s); **renewal anywhere, the same second** (the refresh token is revoked); an access token already cached **on another machine, up to 2 hours** — it is a self-contained JWT and the platform runs no denylist. After a plain `/kabo-logout` only the first of those applies; nothing else changes anywhere.
...
- **Verification reporting is best effort**: `bin/skill-verify` prints `KABO_VERIFY_FAIL` and buffers the allowed event metadata. SessionStart runs `bin/kabo-headers --relay` to report it and prune confirmed entries; failed deliveries wait in the queue. The model is never asked to relay events. This telemetry is a quality signal, not a security audit trail: signature verification failure still means exit 1, and the skill is not executed.
...
└── commands/                     # /kabo-login /kabo-start /kabo-analyze /kabo-channel /kabo-logout /kabo-revoke
```
